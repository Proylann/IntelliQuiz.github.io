    ...
    class QuizSection : AppCompatActivity() {
        ...
        private var maxQuestions = 10  // Default value
        ...
        private fun fetchNextQuestion() {
            if (questions.isEmpty()) {
                val subject = intent.getStringExtra("SUBJECT") ?: "Question"
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    var attemptCount = 0
                    val maxAttempts = 3
                    val questionBatch = mutableListOf<QuizQuestion>()

                    // Fetch a batch of questions based on maxQuestions set by difficulty
                    val numQuestionsToFetch = maxQuestions
                    try {
                        for (i in 0 until numQuestionsToFetch) {
                        ...
                        // Shuffle questions to avoid predictable order
                        questionBatch.shuffle()

                        // Assign the shuffled questions to the global list
                        questions.addAll(questionBatch)
